...
def find_shooter_and_jump(h5_file_path, shot_frame, max_lookback=200, distance_threshold=2, jump_detection_window=60):
    ...
    Includes continuity check to ensure the detected player exists in adjacent frames.
    If best candidate fails continuity check, falls back to next best candidate.
...
def verify_ball_above_rim(player_h5_path, shot_frame, rim_position=None, lookback_frames=8):
    """
    Verify if the ball was above the rim in the frames leading up to the shot.
    ...
    try:
        with h5py.File(player_h5_path, 'r') as h5_file:
            ...
            for frame_idx in range(max(0, shot_frame - lookback_frames), shot_frame + 1):
                ...
                # Get ball position
                ball_y, ball_x = np.where(ball_mask)
                if len(ball_y) == 0:
                    continue
                
                # Calculate ball's vertical position (top point)
                ball_top_y = np.min(ball_y)  # Minimum y is the top in image coordinates
                
                # Calculate height difference between ball and rim
                # Negative value means ball is above the rim
                height_diff = ball_top_y - rim_y
...
def check_shot_validity(court_h5_path, player_h5_path, shot_frame, lookback_frames=8):
    """
    Check if a shot is valid by verifying if the ball was above the rim.
    ...
    # Then check if the ball was above the rim
    is_valid_shot, max_height_frame, max_height_diff = verify_ball_above_rim(
        player_h5_path, shot_frame, rim_position, lookback_frames
    )
...
def create_visualization_with_court(court_h5_path, player_h5_path, frame_num, shooter_id, output_path, 
                                   ...
                                   shooter_detection_method=None, shooter_ball_distance=None):
        ...
        with h5py.File(player_h5_path, 'r') as player_h5:
            ...
            if player_frame_key in player_h5:
                ...
                for obj_key in frame_group.keys():
                    ...
                    color = (0, 165, 255) if obj_key == 'ball' else \
                           (0, 255, 255) if (obj_key == shooter_id and is_jump_frame) else \
                    ...
                    if obj_key == shooter_id:
                        ...
                        if len(player_pixels[0]) > 0:
                            ...
                            if shooter_frame is not None and frame_num == shooter_frame:
                                ...
                                # If we have ball information, draw connection
                                if 'ball' in frame_group:
                                    ball_mask = get_mask_from_player_h5(frame_group, 'ball')
                                    if ball_mask is not None and np.any(ball_mask):
                                        ball_y, ball_x = np.where(ball_mask)
                                        if len(ball_y) > 0:
                                            ball_center_y = int(np.mean(ball_y))
                                            ball_center_x = int(np.mean(ball_x))
...
def create_jump_sequence_video(court_h5_path, player_h5_path, jump_frame, shot_frame, shooter_id, output_path, fps=10, pre_jump_frames=20, jersey_data=None, detection_method=None, shooter_frame=None, shooter_detection_method=None, shooter_ball_distance=None):
...
def find_shooter_and_jump_with_team_verification(h5_file_path, shot_frame, court_h5_path, video_path, expected_team_looking_left, expected_team_looking_right, max_lookback=200, distance_threshold=2, jump_detection_window=60):
    """
    Find the shooter and detect the jump with team verification.
    
    Args:
        h5_file_path: Path to the player tracking h5 file
        shot_frame: Frame where the shot was detected
        court_h5_path: Path to the court tracking h5 file
        expected_team_looking_left: Expected team color for the team looking left (e.g., 'Blue')
        expected_team_looking_right: Expected team color for the team looking right (e.g., 'Red')
        max_lookback: Maximum number of frames to look back for shooter detection
        distance_threshold: Maximum distance between ball and player to be considered the shooter
        jump_detection_window: Maximum number of frames to look back for jump detection
    ...
    Returns:
        ...
        object_id: The object_id attribute of the shooter
        detection_method: String indicating which method was used to detect the jump
        is_team_match: Boolean indicating if the shooter matches the expected team for the shot direction
        shooter_frame: Frame where the shooter was identified
        shooter_detection_method: Method used to identify the shooter
        shooter_ball_distance: Distance between the shooter and ball at identification
    """
    # First, use the existing function to find the shooter and jump
    shooter_id, jump_frame, jump_position, jump_frames, object_id, detection_method = find_shooter_and_jump(
        h5_file_path, shot_frame, max_lookback, distance_threshold, jump_detection_window
    )
    ...
    with h5py.File(h5_file_path, 'r') as h5_file:
        ...
        for frame_idx in range(shot_frame, max(0, shot_frame - max_lookback), -1):
            ...
            if 'ball' not in frame_group or shooter_id not in frame_group:
                continue
            
            ball_mask = get_mask_from_player_h5(frame_group, 'ball')
            player_mask = get_mask_from_player_h5(frame_group, shooter_id)
            
            if ball_mask is None or player_mask is None or not np.any(ball_mask) or not np.any(player_mask):
                continue
...
def main():
    ...
    expected_team_looking_left = "Gray"  # When looking left, the team shooting should be Blue
    expected_team_looking_right = "Blue"  # When looking right, the team shooting should be Red
    ...
    for shot_frame in sequence_starts:
        ...
        is_valid_shot, validation_data = check_shot_validity(court_h5_path, player_h5_path, shot_frame, lookback_frames=8)
        ...
        if is_valid_shot:
            ...
            create_visualization_with_court(court_h5_path, player_h5_path, shot_frame, None, invalid_viz_path)
            ...
            if validation_data['rim_found'] and validation_data['max_height_frame'] is not None:
                ...
                create_visualization_with_court(court_h5_path, player_h5_path, validation_data['max_height_frame'], None, max_height_viz_path)
        ...
        if shooter_id is not None:
            ...
            if jump_frame is not None:
                ...
                create_visualization_with_court(court_h5_path, player_h5_path, jump_frame, shooter_id, team_viz_path, True, jersey_data)
